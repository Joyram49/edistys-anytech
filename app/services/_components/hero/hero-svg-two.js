function HeroServiceSVGTwo() {
  return (
    <div className='lg:hidden'>
      <svg
        xmlns='http://www.w3.org/2000/svg'
        fill='none'
        className='absolute inset-0 h-full w-full z-10'
        preserveAspectRatio='xMidYMid slice'
        style={{
          background:
            "radial-gradient(112.3% 169.97%at -9.66% 166.95%,#00e9ea 0,#1f80f0 52.08%,#005bc3 100%)",
        }}
        viewBox='0 0 797 645'
      >
        <g id='hero-background-mobile'>
          <g id='dark'>
            <g id='Group' style={{ mixBlendMode: "multiply" }}>
              <path
                id='Vector'
                fill='url(#paint0_linear_6_180)'
                d='M797 437.791V345L497 645h92.791z'
              ></path>
            </g>
            <g id='Group_2' style={{ mixBlendMode: "multiply" }}>
              <path
                id='Vector_2'
                fill='url(#paint1_linear_6_180)'
                d='M409.788 305V109.212L0 519.004h195.789z'
              ></path>
            </g>
            <g id='Group_3' style={{ mixBlendMode: "multiply" }}>
              <path
                id='Vector_3'
                fill='url(#paint2_linear_6_180)'
                d='M684 365.595V0L39 644.991h365.596z'
              ></path>
            </g>
          </g>
          <g id='light' clipPath='url(#clip0_6_180)'>
            <g id='Group_4' filter='url(#filter0_f_6_180)'>
              <path
                id='Vector_4'
                fill='url(#paint3_linear_6_180)'
                d='M684 272.067V200L451 433h72.068z'
              ></path>
            </g>
            <g id='Group_5' filter='url(#filter1_f_6_180)'>
              <path
                id='Vector_5'
                fill='url(#paint4_linear_6_180)'
                d='M706.1 69 485 290.1h15.8L706.1 84.8z'
              ></path>
            </g>
            <g id='Group_6' filter='url(#filter2_f_6_180)'>
              <path
                id='Vector_6'
                fill='url(#paint5_linear_6_180)'
                d='M111 313h173L111 486z'
              ></path>
              <path
                id='Vector_7'
                fill='url(#paint6_linear_6_180)'
                d='M111 313h173L111 486z'
              ></path>
            </g>
          </g>
        </g>
        <defs>
          <linearGradient
            id='paint0_linear_6_180'
            x1='366'
            x2='707'
            y1='923'
            y2='519.5'
            gradientUnits='userSpaceOnUse'
          >
            <stop stopColor='#1F80F0'></stop>
            <stop offset='1' stopColor='#0059BF' stopOpacity='0'></stop>
          </linearGradient>
          <linearGradient
            id='paint1_linear_6_180'
            x1='-276.407'
            x2='443.103'
            y1='1105.58'
            y2='254.197'
            gradientUnits='userSpaceOnUse'
          >
            <stop stopColor='#1F80F0'></stop>
            <stop offset='1' stopColor='#0059BF' stopOpacity='0'></stop>
          </linearGradient>
          <linearGradient
            id='paint2_linear_6_180'
            x1='971.501'
            x2='401'
            y1='1062'
            y2='388.5'
            gradientUnits='userSpaceOnUse'
          >
            <stop stopColor='#1F80F0'></stop>
            <stop offset='1' stopColor='#0059BF' stopOpacity='0'></stop>
          </linearGradient>
          <linearGradient
            id='paint3_linear_6_180'
            x1='408.636'
            x2='678.393'
            y1='452.001'
            y2='221.805'
            gradientUnits='userSpaceOnUse'
          >
            <stop stopColor='#00E9EA'></stop>
            <stop offset='1' stopColor='#1F80F0' stopOpacity='0.49'></stop>
          </linearGradient>
          <linearGradient
            id='paint4_linear_6_180'
            x1='444.8'
            x2='700.779'
            y1='308.131'
            y2='89.691'
            gradientUnits='userSpaceOnUse'
          >
            <stop stopColor='#00E9EA'></stop>
            <stop offset='1' stopColor='#1F80F0' stopOpacity='0.49'></stop>
          </linearGradient>
          <linearGradient
            id='paint5_linear_6_180'
            x1='111'
            x2='284'
            y1='399.15'
            y2='399.15'
            gradientUnits='userSpaceOnUse'
          >
            <stop stopColor='#fff'></stop>
            <stop offset='1' stopColor='#4B4B4B' stopOpacity='0'></stop>
          </linearGradient>
          <linearGradient
            id='paint6_linear_6_180'
            x1='6.971'
            x2='251.76'
            y1='513.992'
            y2='258.952'
            gradientUnits='userSpaceOnUse'
          >
            <stop stopColor='#00E9EA'></stop>
            <stop offset='1' stopColor='#1F80F0'></stop>
          </linearGradient>
          <filter
            id='filter0_f_6_180'
            width='295'
            height='295'
            x='420'
            y='169'
            colorInterpolationFilters='sRGB'
            filterUnits='userSpaceOnUse'
          >
            <feFlood floodOpacity='0' result='BackgroundImageFix'></feFlood>
            <feBlend
              in='SourceGraphic'
              in2='BackgroundImageFix'
              result='shape'
            ></feBlend>
            <feGaussianBlur
              result='effect1_foregroundBlur_6_180'
              stdDeviation='15.5'
            ></feGaussianBlur>
          </filter>
          <filter
            id='filter1_f_6_180'
            width='245.1'
            height='245.1'
            x='473'
            y='57'
            colorInterpolationFilters='sRGB'
            filterUnits='userSpaceOnUse'
          >
            <feFlood floodOpacity='0' result='BackgroundImageFix'></feFlood>
            <feBlend
              in='SourceGraphic'
              in2='BackgroundImageFix'
              result='shape'
            ></feBlend>
            <feGaussianBlur
              result='effect1_foregroundBlur_6_180'
              stdDeviation='6'
            ></feGaussianBlur>
          </filter>
          <filter
            id='filter2_f_6_180'
            width='239'
            height='239'
            x='78'
            y='280'
            colorInterpolationFilters='sRGB'
            filterUnits='userSpaceOnUse'
          >
            <feFlood floodOpacity='0' result='BackgroundImageFix'></feFlood>
            <feBlend
              in='SourceGraphic'
              in2='BackgroundImageFix'
              result='shape'
            ></feBlend>
            <feGaussianBlur
              result='effect1_foregroundBlur_6_180'
              stdDeviation='16.5'
            ></feGaussianBlur>
          </filter>
          <clipPath id='clip0_6_180'>
            <path fill='#fff' d='M78 57h641v462H78z'></path>
          </clipPath>
        </defs>
      </svg>
      <div
        style={{
          background: "linear-gradient(to right, #00E9EA, #0D7DFE)",
          clipPath: "polygon(0 98%, 100% 82%, 100% 100%, 0 100%)",
        }}
        className='h-full w-full absolute bottom-0 z-40'
      ></div>
    </div>
  );
}

export default HeroServiceSVGTwo;
